...
export const DimensionTool = observer(() => {
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (key === 'x') {
        selectedWall.dimension.setLockedAxis(selectedWall.dimension.lockedAxis === 'x' ? 'none' : 'x');
      } else if (key === 'y') {
        selectedWall.dimension.setLockedAxis(selectedWall.dimension.lockedAxis === 'y' ? 'none' : 'y');
      }
    };
